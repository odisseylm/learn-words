...
import jdk.incubator.foreign.LibraryLookup as JILibraryLookup
import jdk.incubator.foreign.LibraryLookup.Symbol as JINativeSymbol
...
See other docs
 https://docs.oracle.com/en/java/javase/16/docs/api/jdk.incubator.foreign/jdk/incubator/foreign/package-summary.html
 https://docs.oracle.com/en/java/javase/16/docs/api/jdk.incubator.foreign/jdk/incubator/foreign/CLinker.html
...
class Linker (private val delegate: JICLinker) {
    ...
    fun downcallHandle(address: MemoryAddress, function: FunctionDescriptor): MethodHandle {
        ...
        return MethodHandle(delegate.downcallHandle(address.symbol, methodHandle, nativeFC))
    }
    ...
    fun systemLookup(): SymbolLookup = SymbolLookup(JILibraryLookup.ofDefault())
...
class SymbolLookup (
    private val delegate: JILibraryLookup,
    private val fallback: JILibraryLookup? = null,
    ) {
    ...
    companion object {
        ...
        fun defaultLookup(): SymbolLookup =
            SymbolLookup(JILibraryLookup.ofDefault())
        ...
        fun libraryLookup(libName: String, arena: Arena): SymbolLookup {
            return SymbolLookup(JILibraryLookup.ofLibrary(libName.removeSuffix(".dll")))
        }
    }
}
...
class Arena internal constructor (
    ...
    private val resourceScope = ResourceScope.of(resourceScopeType)
    ...
    fun allocate(byteSize: Long): MemorySegment =
        MemorySegment(JIMemorySegment.allocateNative(byteSize))
            .also { resourceScope.addResource(it.delegate) }

    fun allocate(layout: MemoryLayout): MemorySegment =
        MemorySegment(JIMemorySegment.allocateNative(layout.delegate))
            .also { resourceScope.addResource(it.delegate) }

    fun allocate(layout: MemoryLayout, count: Long): MemorySegment =
        MemorySegment(JIMemorySegment.allocateNative(layout.delegate.byteSize() * count))
            .also { resourceScope.addResource(it.delegate) }
    ...
    companion object {
        ...
        fun ofConfined(): Arena =
            Arena(toClose = true, ResourceScopeType.Confined)
        @JvmStatic
        fun ofAuto(): Arena =
            Arena(toClose = false, ResourceScopeType.Auto)
        //@JvmStatic
        //fun ofShared(): Arena =
        //    Arena(toClose = false, ResourceScopeType.Shared)
        @JvmStatic
        fun global(): Arena =
            Arena(toClose = false, ResourceScopeType.Global)
    }
}
...
open class MemoryLayout (private val delegate0: JIMemoryLayout) {
    ...
    companion object {
        ...
        fun structLayout(vararg layouts: MemoryLayout): StructLayout {
            val sl: JIGroupLayout = JIMemoryLayout.ofStruct(
                *layouts.map { it.delegate }.toTypedArray()
            )
            return StructLayout(sl)
        }

        fun paddingLayout(byteSize: Long): MemoryLayout = MemoryLayout(JIMemoryLayout.ofPaddingBits(byteSize * 8))
    }
}
...
internal enum class ResourceScopeType { Auto, Confined, Global, Shared }

internal class ResourceScope (private val resourceScopeType: ResourceScopeType) {
    private val resources: MutableList<AutoCloseable> =
        if (resourceScopeType.isOneOf(ResourceScopeType.Global, ResourceScopeType.Shared)) synchronizedList(mutableListOf())
        else mutableListOf()
    ...
    companion object {
        fun of(type: ResourceScopeType) = when (type) {
            ResourceScopeType.Confined -> ofConfined()
            ResourceScopeType.Auto     -> ofAuto()
            ResourceScopeType.Shared   -> ofShared()
            ResourceScopeType.Global   -> global()
        }

        private fun ofConfined(): ResourceScope = ResourceScope(ResourceScopeType.Confined)
        private fun ofAuto():     ResourceScope = ResourceScope(ResourceScopeType.Auto)
        private fun ofShared():   ResourceScope = ResourceScope(ResourceScopeType.Shared)
        private fun global():     ResourceScope = globalResourceScope
    }

    fun close() {
        if(resourceScopeType != ResourceScopeType.Global)
            resources.toTypedArray().forEach {
                try { it.close() }
                catch (ex: Exception) {
                    log.error(ex) { "Error of closing foreign resource" }
                }
            }
    }
}
